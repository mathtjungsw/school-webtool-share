...
- 현재 통합 릴리스는 데스크톱 v1.1.34, 모바일 PWA v1.1.34, Apps Script `MOBILE_SERVICE_VERSION = 49`를 기준으로 한다. 이후 변경 시 버전을 올리며, 현재 실제 원격 버전보다 내려가지 않는다.
...
- 학급 전체를 수업 명단으로 간주하지 않는다. 공유 `학생시간표`의 학생별 당일 교시에서 로그인 교사와 일치하는 3학년 학생만 실제 수강생으로 선별한다.
