...
기준일·최종 확인일: 2026-09-19. 출처: 최신 `origin/main`의 웅천고 업무도우미, 기존 고정 Apps Script 소스, 3학년 실시간 출결 시트 구조, 모바일 일정 PWA 구현·회귀검사. 이 문서는 데스크톱과 모바일을 하나의 배포 계약으로 유지하기 위한 개발 지침이다.
...
- 현재 통합 릴리스는 데스크톱 v1.1.33, 모바일 PWA v1.1.32, Apps Script `MOBILE_SERVICE_VERSION = 48`을 기준으로 한다. 이후 변경 시 버전을 올리며, 현재 실제 원격 버전보다 내려가지 않는다.
...
모바일·보호메뉴 통합에서 변경을 허용하는 범위는 `doGet`·`doPost`, `mobile*`, `executive*`, `getMobileScheduleBundle_`, `MOBILE_*`·`EXECUTIVE_*` 상수, `RELEASE_NOTES`다. 이 외의 데스크톱 함수는 본문 전체, 상수는 값·초깃값 표현식·선언 형태를 최신 `origin/main`과 비교한다. `listStaffChecklists_` 이름만 유지하고 `return []`로 바꾸거나 `STAFF_CHECKLISTS_SHEET`를 다른 시트명으로 바꾸는 변경도 차단한다. 줄바꿈 형식과 주석을 제외한 코드 정의를 기준으로 비교하며, 새 로컬 데스크톱 함수/상수가 main이나 실제 원격 어느 쪽에도 없으면 검토되지 않은 변경으로 차단한다.
