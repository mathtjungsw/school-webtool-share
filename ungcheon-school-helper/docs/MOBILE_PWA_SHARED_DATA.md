...
기준일·최종 확인일: 2026-09-14. 출처: 최신 `origin/main`의 웅천고 업무도우미, 기존 고정 Apps Script 소스, 3학년 실시간 출결 시트 구조, 모바일 일정 PWA 구현·회귀검사. 이 문서는 데스크톱과 모바일을 하나의 배포 계약으로 유지하기 위한 개발 지침이다.
...
- 현재 통합 릴리스는 데스크톱 v1.1.32, 모바일 PWA v1.1.31, Apps Script `MOBILE_SERVICE_VERSION = 47`을 기준으로 한다. 이후 변경 시 버전을 올리며, 현재 실제 원격 버전보다 내려가지 않는다.
