...
기준일·최종 확인일: 2026-09-21. 출처: 최신 `origin/main`의 웅천고 업무도우미, 기존 고정 Apps Script 소스, 3학년 실시간 출결 시트 구조, 모바일 일정 PWA·바탕화면 위젯 구현·회귀검사. 이 문서는 데스크톱과 모바일을 하나의 배포 계약으로 유지하기 위한 개발 지침이다.
...
- 현재 통합 릴리스는 데스크톱 v1.1.34, 모바일 PWA v1.1.35, Apps Script `MOBILE_SERVICE_VERSION = 50`을 기준으로 한다. 이후 변경 시 버전을 올리며, 현재 실제 원격 버전보다 내려가지 않는다.
...
- 전체 학생 명렬·전체 학생 시간표·학번·원본 학생시간표 payload, NEIS 학사일정·NEIS 학급시간표는 모바일 응답에 포함하지 않는다. 3학년 수강생 출결 기능은 서버 안에서만 학생시간표를 대조하며, 로그인 교사의 당일 수업에 실제로 등록된 수강생 중 출결 비고가 있는 학생의 반·번호·이름·비고와 반별 입력 완료 상태만 최소 응답한다.
...
- 출결 시트와 수강생은 원소속 반·번호를 우선 키로 대조하고 이름까지 일치해야 표시한다. 불일치 항목은 상세 목록에서 제외하고 확인 필요 건수만 알린다.
- 관련 반이 모두 입력되면 `입력 완료`, 일부만 입력되면 `부분 입력`, 모두 미입력이면 `입력 전`으로 표시한다. 일부 반이 입력 전이어도 완료된 반의 출결 비고는 확인할 수 있다.
- 모바일 상세 목록은 `반·번호 | 이름 | 출결 비고` 순서다. 출결 비고가 비어 있는 수강생과 해당 수업을 듣지 않는 학생은 응답하지 않는다.
...
모바일·보호메뉴 통합에서 변경을 허용하는 범위는 `doGet`·`doPost`, `mobile*`, `executive*`, `getMobileScheduleBundle_`, `getWidgetAttendanceSummaries_`, `MOBILE_*`·`EXECUTIVE_*` 상수, `RELEASE_NOTES`다. 이 외의 데스크톱 함수는 본문 전체, 상수는 값·초깃값 표현식·선언 형태를 최신 `origin/main`과 비교한다. `listStaffChecklists_` 이름만 유지하고 `return []`로 바꾸거나 `STAFF_CHECKLISTS_SHEET`를 다른 시트명으로 바꾸는 변경도 차단한다. 줄바꿈 형식과 주석을 제외한 코드 정의를 기준으로 비교하며, 새 로컬 데스크톱 함수/상수가 main이나 실제 원격 어느 쪽에도 없으면 검토되지 않은 변경으로 차단한다.
