# 모바일 일정 PWA 공유 데이터 계약

기준일·최종 확인일: 2026-09-22. 출처: 최신 `origin/main`의 웅천고 업무도우미, 기존 고정 Apps Script 소스, 3학년 실시간 출결 시트 구조, 모바일 일정 PWA·바탕화면 위젯 구현·회귀검사. 이 문서는 데스크톱과 모바일을 하나의 배포 계약으로 유지하기 위한 개발 지침이다.

## 유지할 공개 주소와 기반

- 데스크톱 통합은 최신 `origin/main` 기준으로 작업한다. v1.1.13 기반 과거 detached 작업트리를 그대로 배포하지 않는다.
- Apps Script project: `1rLmGrYBAeMUHxGr9CSeT1DUtTUoDbmNpydp1rGe5DL-RzDlyoZCleLnb`.
- 고정 deployment: `AKfycbwFiXk0fxkJSy2Mk17BPKblEARQZYdAUzP6JDtpbV_Qj203xHGWqxnBqSaWaWJYDOyu4w`.
- Apps Script URL: <https://script.google.com/macros/s/AKfycbwFiXk0fxkJSy2Mk17BPKblEARQZYdAUzP6JDtpbV_Qj203xHGWqxnBqSaWaWJYDOyu4w/exec>.
- 모바일 PWA 공개 URL: <https://ungcheon-mobile-schedule.jsw890122.chatgpt.site>. PWA 배포 설정과 데스크톱 환경설정도 이 주소를 그대로 사용한다. 새 사이트/호스트/주소로 바꾸지 않는다.
- 이번 통합 릴리스는 데스크톱 v1.1.35, 모바일 PWA v1.1.36, Apps Script `MOBILE_SERVICE_VERSION = 51`을 기준으로 한다. 실제 공개 배포 확인 결과는 각 릴리스 검증 문서에 기록한다.

## 로그인과 응답

- `verifyMobileViewer`: 교직원 명렬의 이름과 기존 공통 비밀번호를 확인한다.
- 세션은 72시간이다. `UNG_MOBILE_SHARED_PASSWORD_HASH`, `UNG_MOBILE_SESSION_` 접두사 및 현재 유효한 세션을 보존한다. 코드 배포에 비밀번호 변경/속성 초기화 작업을 섞지 않는다.
- `getMobileScheduleBundle`: 로그인한 교사의 읽기 전용 자료를 제공한다. `contractVersion: 3`과 `sourceStatus`를 유지한다.
- `getMobileAttendanceRoster`: 로그인 세션과 교사·날짜·교시를 다시 대조한 뒤, 출결 버튼을 누른 한 수업의 실제 수강생 `반·번호·이름·비고`만 반환한다. 일정 묶음에는 전체 명렬을 포함하지 않는다.
- 출처는 주간계획, 창체, 창체 학사일정, 등교지도, 급식지도, 교사 시간표, 내 위원회, 승인·반영된 수업 변경, 일일 시간표 예외, 급식, 3학년 수강생 출결이다.
- `timetableOverrides`는 공유 `일일시간표예외` 시트의 활성 자료만 요청 범위에 맞춰 전달한다. 모바일은 이 자료를 교사 주간시간표와 당김수업에 덮어쓰며 직접 수정하지 않는다.
- 각 출처는 독립 오류 경계로 읽는다. `fresh`는 자료가 있는 정상 응답, `empty`는 정상 0건, `unavailable`은 해당 출처 읽기 실패다. 한 출처 실패가 전체 조회 실패가 되어서는 안 된다.
- 전체 학교 학생 명렬·전체 학생 시간표·학번 원본·원본 학생시간표 payload, NEIS 학사일정·NEIS 학급시간표는 모바일 응답에 포함하지 않는다. 일정 묶음에는 출결 비고가 있는 실제 수강생의 최소 요약과 반별 입력 완료 상태만 포함한다. 출결 버튼을 누른 경우에만 별도 인증 응답으로 그 한 수업의 실제 수강생 반·번호·이름·비고를 제공한다.
- 개인 일정·개인 업무는 PC에서만 확인 가능하다고 모바일에 안내한다.

## 3학년 수강생 출결

- 출결 원본은 `3학년 실시간 출결확인` 스프레드시트의 `3학년` 탭을 사용한다. 1행 날짜, 2행 반별 입력 완료 체크, 3행부터 반복되는 `반·이름·비고` 구조를 기준으로 읽는다.
- 3학년 수업은 이동수업인지 학급수업인지 구분하지 않고 모두 출결을 연결한다. 학급 전체를 수업 명단으로 간주하지 않으며, 공유 `학생시간표`의 학생별 당일 교시에서 로그인 교사의 해당 과목·교실과 일치하는 3학년 학생만 실제 수강생으로 선별한다.
- 공동수업 교사명은 쉼표 등으로 분리해 개별 교사를 확인한다. 학생시간표의 교사명이 줄어든 경우에는 같은 날짜·교시의 실제 교사시간표와 과목·교실까지 대조해 후보가 한 명일 때만 연결하며, 같은 교시의 다른 과목 수강생을 섞지 않는다.
- 승인·반영된 교체·대강은 이동한 수업의 원래 날짜·교시·교사를 따라 실제 수강생을 다시 찾고, 당김수업은 원래 수업 위치의 수강생을 새 날짜·교시로 옮긴다. 이동형 변경의 원래 칸에는 출결 버튼을 중복 표시하지 않는다.
- 변경수업의 과목·수업반·원래 위치를 확정할 수 없으면 임의의 학급 명렬을 대신 사용하지 않고 `출결 대상 확인 필요`로 멈춘다.
- 출결 시트와 수강생은 원소속 반·번호를 우선 키로 대조하고 이름까지 일치해야 비고를 연결한다. 불일치 학생은 전체 명렬에서 삭제하지 않고 비고를 연결하지 않은 채 유지하며 확인 필요 건수를 함께 알린다.
- 관련 반이 모두 입력되면 `입력 완료`, 일부만 입력되면 `부분 입력`, 모두 미입력이면 `입력 전`으로 표시한다. 일부 반이 입력 전이어도 완료된 반의 출결 비고는 확인할 수 있다.
- 일정 묶음의 최소 출결 요약은 `반·번호 | 이름 | 출결 비고` 순서이며 비고가 있는 실제 수강생만 포함한다. 해당 수업을 듣지 않는 학생은 요약과 상세 어느 응답에도 포함하지 않는다.
- 버튼을 누른 시점에만 `getMobileAttendanceRoster`로 해당 수업 명렬을 조회한다. 명렬은 반·번호 숫자 오름차순을 유지하고 비고가 없으면 `비고 없음`으로 표시한다. `비고만` 전환은 받은 응답을 화면 메모리에서 필터링하므로 추가 요청을 만들지 않는다.
- 출결 상세 전체 명렬은 서비스 워커나 IndexedDB에 저장하지 않으며 팝업을 닫거나 로그아웃하면 메모리에서 지운다. 서버 캐시는 최대 5분이다.
- 출결 비고에 `자습`만 있고 실제 출결 이상 표현이 없는 학생은 결석으로 집계하지 않는다. 모바일과 위젯에서 연두색 `자습 · 장소 이동`으로 분리하며, `자습`과 결석·지각·조퇴·결과·병결·미인정·출석인정·공결·체험학습·위탁교육이 함께 있으면 실제 출결 항목으로 우선 분류한다.
- PWA는 5분마다 자료를 다시 확인한다. 출결 출처만 실패하면 사용자별 기기 캐시의 이전 정상 출결을 `이전 자료`로 구분해 유지하며, 로그아웃 시 해당 사용자 캐시를 삭제한다.
- 데스크톱 위젯은 `getWidgetAttendanceSummaries`로 오늘 자료만 5분 간격 조회한다. 서버는 로그인 이름을 교직원 명렬과 대조하고 모바일과 동일한 실제 수강생·변경수업 계산을 사용하되 반·번호·이름·비고 및 반별 입력 상태만 응답한다. 전체 학생시간표 원본이나 학생 식별키를 데스크톱으로 반환하지 않는다.

## 창체 교시 계약

- 창체 시트의 교시 열은 모바일 일정의 `periodStart`, `periodEnd`로 별도 전달한다. 모바일은 제목 속 학년 숫자를 해석하지 않고 이 명시적 필드를 우선 사용한다.
- 구형 응답은 `5교시`, `5~6교시`, `5·6교시`처럼 숫자 뒤에 `교시`가 직접 붙은 표현만 해석한다. `1·2·3학년`은 교시로 간주하지 않는다.
- 교시 지정 창체는 해당 수업 교시 행에만 표시하고 쉬는 시간·점심시간에는 반복하지 않는다.

## 급식

- 관리자 PC가 학교 공유 스프레드시트의 `NEIS급식` 시트에 이미 동기화한 자료만 읽는다.
- `mobileSharedMealsInRange_(fromDate, toDate)`가 요청 기간 급식을 조회하여 `meals`로 반환한다.
- 급식 필드는 `date`, `mealType`, `dishNames`, `calories`로 제한한다.
- 한국 시간 기준 오늘의 급식을 담는 기존 `todayMeals`도 유지한다. 조회 기간에 오늘이 포함되지 않았더라도 기존 오늘 급식 계약이 깨지지 않는지 확인한다.
- 모바일에서는 NEIS API를 직접 호출하지 않는다. 모바일 응답 캐시 키에는 요청 기간과 한국 시간 기준 오늘 날짜를 함께 넣는다.

## PWA 연결 안정성

- 자료 조회 제한은 35초로 한다. 실제 고정 주소의 첫 조회 소요 시간을 측정한다.
- 로그인 만료는 네트워크 실패와 구분한다. “로그인이 만료되었습니다. 다시 로그인해 주세요.”를 표시하고 로그인 화면으로 전환한다.
- IndexedDB 저장 실패는 API 연결 실패로 바꾸지 않는다. 받은 자료는 화면에 반영하고 캐시 저장 문제만 별도로 처리한다.
- `pageshow`와 `visibilitychange`가 연달아 발생해도 동일 조회가 중복 실행되지 않게 한다. 진행 중 요청과 갱신 시점을 함께 관리한다.
- 서비스 워커는 Apps Script POST를 캐시·가로채기·차단하지 않는다. 인증된 서버 응답을 서비스 워커 캐시에 저장하지 않는다.
- 앱 코드와 화면은 네트워크 최신본을 먼저 확인하고, 서비스 워커가 교체되면 기존 앱 탭을 한 번만 새로고침한다. iOS의 동작 줄이기로 회전 애니메이션이 멈추더라도 조회 경과 초를 표시해 진행 상태를 알린다.

## 배포 전 필수 검사

1. 최신 main을 가져오고 현재 HEAD가 그 커밋을 포함하는지 확인한다.
2. 실제 고정 배포 버전과 원격 HEAD를 읽고, 최신 main과 함께 **코드 diff를 검토**한다. 기존 함수·액션·릴리스 본문 문장이 빠지면 중단한다. 같은 릴리스에 데스크톱/모바일 안내가 있으면 합쳐 보존한다. 함수 이름의 존재만으로 보존을 판단하지 않는다.
3. 데스크톱 `npm run typecheck`, 관련 회귀검사, 모바일 typecheck·전체 단위/통합 테스트·빌드를 통과시킨다.
4. `node scripts/check-apps-script-deploy-guard.cjs`로 배포 가드를 검사한다.
5. `npm run deploy:apps-script -- -ValidateOnly`로 Code.gs 구문·계약을 검사한다. 원격에 접근하지 않는다.
6. `npm run deploy:apps-script -- -PreflightOnly`로 고정 배포·원격 HEAD를 읽기 비교한다. 업로드/배포하지 않는다.
7. 검증한 main을 커밋·푸시한 뒤 정식 스크립트로 기존 fixed deployment만 갱신한다. 업로드 직전 main과 실제 원격 snapshot을 재검사하며, 업로드 후 정확히 같은 코드인지 확인한 불변 프로젝트 버전만 배포한다.
8. 고정 URL health의 서비스 버전, 실제 이름 로그인, 요청 기간 일정·교사 시간표·급식과 3학년 수강생 출결을 확인한다. 35초 안에 첫 조회 완료, 출처별 상태, 부분 입력 판정 및 전체 학생 자료·학번·원본 payload 부재도 확인한다.
9. PWA를 기존 공개 사이트로 배포하고 고정 주소와 기존 세션이 유지되는지 확인한다.

## 배포 가드의 범위와 비밀 보호

모바일·보호메뉴 통합에서 변경을 허용하는 범위는 `doGet`·`doPost`, `mobile*`, `executive*`, `getMobileScheduleBundle_`, `getMobileAttendanceRoster_`, `getWidgetAttendanceSummaries_`, `MOBILE_*`·`EXECUTIVE_*` 상수, `RELEASE_NOTES`다. 이 외의 데스크톱 함수는 본문 전체, 상수는 값·초깃값 표현식·선언 형태를 최신 `origin/main`과 비교한다. `listStaffChecklists_` 이름만 유지하고 `return []`로 바꾸거나 `STAFF_CHECKLISTS_SHEET`를 다른 시트명으로 바꾸는 변경도 차단한다. 줄바꿈 형식과 주석을 제외한 코드 정의를 기준으로 비교하며, 새 로컬 데스크톱 함수/상수가 main이나 실제 원격 어느 쪽에도 없으면 검토되지 않은 변경으로 차단한다.

3-way 비교에서는 최신 main에 이미 정의된 데스크톱 함수/상수를 권위 기준으로 삼는다. 실제 원격이 과거 버전이어도 로컬 정의가 최신 main과 같으면 main에서 정식 검토·반영한 개선을 허용한다. 반대로 main에 없고 원격에만 있는 데스크톱 정의는 원격 그대로 보존해야 한다. 이 정책은 원격과 main의 의미상 차이를 자동으로 판단한다는 뜻이 아니므로, 배포자는 사전 코드 diff에서 차이의 근거를 확인해야 한다. 의도적인 별도 데스크톱 수정은 테스트와 main 반영 후 다시 사전 검사를 수행하며 이 가드의 예외 목록을 임의로 넓혀 우회하지 않는다.

`scripts/deploy-apps-script.ps1`은 기존 clasp OAuth를 메모리에서만 재사용하여 Apps Script API를 호출한다. 프로젝트 파일 업로드·불변 버전 생성·기존 배포 갱신만 수행하고 ScriptProperties API나 서버 초기화 함수를 호출하지 않는다. OAuth·공통 비밀번호·사용자 세션 값은 출력하지 않는다. 원격 소스도 로그/임시 파일에 저장하지 않고 메모리에서 비교한다.

Apps Script 배포 API에는 원자적인 비교-교환 잠금이 없으므로 같은 프로젝트에 동시 수동 배포하지 않는다. 스크립트는 여러 단계에서 원격 버전과 코드 해시를 재확인하고 변경이 감지되면 중단하지만, 감지 직후의 극히 짧은 동시 갱신 구간까지 서버 잠금으로 보장하지는 않는다. 중단 후에는 자동 덮어쓰기나 임의 롤백 대신 원격 변경을 다시 검토한다.
