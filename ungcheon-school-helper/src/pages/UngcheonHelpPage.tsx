...
const SECTIONS = [
  {
    title: '미니 위젯 · 수업과 시간 지정 일정',
    icon: CalendarDays,
    items: [
      '상단 미니 위젯 버튼으로 열면 오늘 시간표 왼쪽에 수업, 오른쪽에 시간이 지정된 일정이 표시됩니다.',
      '교시·시각·수업은 한 줄로 표시합니다. 긴 수업명이나 변경 표시는 수업 칸을 눌러 전체 내용을 확인합니다.',
      '각 기능 제목 오른쪽의 화살표를 누르면 해당 기능만 한 줄 요약으로 접힙니다. 접힘 상태는 이 PC에 저장되며, 순서를 바꾸거나 다시 실행해도 유지됩니다.',
      '기능 접기는 설정의 숨기기나 상단의 위젯 전체 접기와 별개입니다. 기능을 접었다 펼쳐도 작성 중인 메모·도구 값과 뽑은 카드는 유지됩니다.',
      '시작·종료 시간이 있는 위원회·일정은 해당 시간의 영역으로, 시각만 있는 업무·일정은 단일 시각 태그로 표시합니다. 점심·쉬는시간·수업 전후 일정도 확인할 수 있습니다.',
      '일정을 누르면 제목·날짜·시간·장소를 위젯 안에서 확인합니다. 같은 시간에 여러 일정이 겹치면 펼쳐 보기로 모두 확인합니다.',
      '기존 오늘 주요 일정 영역은 시간표로 통합했습니다. 종일 일정과 날짜만 있는 업무는 시간표에 표시하지 않지만 기존 캘린더·업무센터의 원본은 그대로 유지됩니다.',
      '설정의 시간표의 시간 지정 일정 표시와 종류별 체크박스로 조절합니다. 대시보드 체크박스와는 별개입니다. 시간 없는 NEIS 학사일정 등은 체크해도 시간표 영역에 표시되지 않습니다.',
      '내용이 화면 높이를 넘으면 본문만 스크롤하고 상단·하단 버튼은 계속 보입니다. 핀은 항상 위에 표시를 켜고 끄는 버튼이며, 접으면 간단한 현재·다음 수업과 건수만 표시됩니다.',
    ],
  },
  {
    title: '처음 시작하기',
  ...
  },
  {
    ...
    items: [
      ...
      '업무 시작·마감에는 날짜와 선택형 시간 또는 교시를 함께 입력할 수 있습니다. 시간을 모두 비우면 종일 업무이며, 교시 입력은 학교 일과시간으로 자동 환산되어 오늘 시간표·위젯·캘린더에 표시됩니다.',
  ...
  },
  {
    ...
    items: [
      ...
      '후보 교사를 누르면 후보 교사 목록 바로 위에 예상 시간표가 표시되어 교환·대강 적용 전후의 주간 시간표와 연강 변화를 미리 확인할 수 있습니다.',
      ...
      '시간표 업무 담당자는 “일일 시간표 예외 관리”에서 특정 날짜에 다른 날짜의 수업을 가져오거나 교시를 비울 수 있습니다. 저장한 예외는 구글시트의 일일시간표예외 시트와 데스크톱·모바일 시간표에 함께 반영됩니다.',
      '2026년 9월 11일 7교시와 9월 23일 교시 이동은 초기 공통 예외로 등록되며, 이후 일회성 운영도 프로그램 재배포 없이 관리자 화면이나 공유 시트에서 변경할 수 있습니다.',
  ...
  },
  {
    ...
    items: [
      ...
      '학생 위치 찾기에서는 1·2·3학년 학생을 학번 또는 이름으로 검색해 현재 교시의 수업·교실·담당 교사를 확인합니다. 도움반 학생은 개인 시간표에서 색칠된 수업의 위치를 도움반으로 안내하며, 색칠된 동아리는 도움반으로 처리하지 않습니다. 1학년은 학생 명렬과 공용 학급시간표를 함께 사용하고 동명이인은 학번·학급으로 선택하며 승인된 교환·대강도 반영됩니다.',
      '화면은 밝은 모드로 고정되며, 본문·안내문·표·배지는 밝은 배경에서 어두운 글자로 선명하게 표시됩니다.',
