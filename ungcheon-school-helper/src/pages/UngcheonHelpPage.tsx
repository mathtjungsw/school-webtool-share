...
const SECTIONS = [
  ...
  {
    title: '업무 체크리스트·교원 명렬',
    icon: FileSpreadsheet,
    items: [
      '로그인 도입 전에는 환경설정의 교사 이름을 체크리스트 작성자와 응답자 확인 기준으로 사용합니다.',
      '관리자가 교원 명렬 Excel을 한 번 등록하면 교장·교감이 먼저, 나머지 교원이 가나다순으로 표시됩니다.',
      '체크리스트 작성자는 전체·부서·개별 교원을 선택해 배부하고 교원별 완료 현황을 확인할 수 있습니다.',
  ...
  },
  {
    ...
    items: [
      ...
      '성적 산출 미리보기에서는 시험 전 희망 추정분할점수에 맞는 난이도별 예상 정답률을 설계하고, 1차 시험 성적 Excel·2차 시험 가정값·수행평가를 합산해 석차등급과 성취도 분포를 확인할 수 있습니다. 희망 성취도 분포를 입력하면 2차 시험 추정분할점수와 정답률도 역산합니다. 선택형 배점이 0점이면 선택형은 계산에서 완전히 제외되며 서술형 정답률만 제시됩니다. 복원용 정리 Excel로 작업을 옮길 수 있고, 점수 자료는 학교 공유 서버로 전송되지 않습니다.',
