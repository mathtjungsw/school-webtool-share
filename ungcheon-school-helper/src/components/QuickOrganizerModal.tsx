...
export function QuickOrganizerModal({ date, onClose, onSaved }: {
  ...
  useEffect(() => {
    if (!date) return
    setTargetDate(date); setTitle(''); setTime(''); setMemo(''); setMode('schedule'); setShowOnCalendar(true)
  }, [date])
  if (!date) return null
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await savePersonalTasks([...tasks, {
        ...
        time: time || undefined, priority, completed: false, memo: memo.trim(),
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/65 p-4" onMouseDown={event => { if (event.currentTarget === event.target) onClose() }}>
    <form onSubmit={save} className="card w-full max-w-lg border-violet-400/25 p-5 shadow-2xl">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2"><label><span className="mb-1 block text-[10px] font-bold text-slate-400">{mode === 'schedule' ? '날짜' : '마감일'}</span><input type="date" required value={targetDate} onChange={event => setTargetDate(event.target.value)} /></label><label><span className="mb-1 block text-[10px] font-bold text-slate-400">시간</span><input type="time" value={time} onChange={event => setTime(event.target.value)} /></label></div>
