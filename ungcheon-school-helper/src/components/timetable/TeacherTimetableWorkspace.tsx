...
function LessonLine({ lesson, expanded = false }: { lesson: CompositeTeacherDay['lessons'][number]; expanded?: boolean }) {
  return <div className={clsx('min-w-0', expanded && 'flex items-start gap-2')}><span className="text-[9px] font-black text-slate-500">{lesson.period}교시</span>{lesson.value ? <span className={clsx('block truncate text-[9px] font-bold', lesson.source === 'base' ? 'text-slate-300' : lesson.source === 'pulled' ? 'text-emerald-300' : 'text-violet-300')} title={oneLine(lesson.value)}>{oneLine(lesson.value)}</span> : <span className="block text-[9px] text-slate-600">공강</span>}{lesson.badge && <span className="mt-0.5 block text-[8px] font-black text-amber-300">{lesson.badge}</span>}</div>
}
