...
const KIND_STYLE: Record<
  TodayBriefingKind,
  { icon: typeof CalendarDays; badge: string; label: string }
> = {
  schedule: {
    icon: CalendarDays,
  ...
  system: {
    icon: AlertTriangle,
    badge: "bg-rose-100 text-rose-950 dark:bg-rose-500/20 dark:text-rose-100",
    label: "점검",
  },
};
...
export function TodayBriefingWidget({
  ...
  const visible = useMemo(
    () =>
      (tab === "all" ? items : items.filter((item) => item.kind === tab)).slice(
        0,
        tab === "all" ? 6 : 10,
      ),
    [items, tab],
  ...
  return (
    <section
      className="card mb-4 overflow-hidden border-sky-300/60 bg-white/85 p-0 dark:border-sky-500/20 dark:bg-surface-900/85"
      aria-label="오늘 브리핑"
    >
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 px-3 py-2 dark:border-white/10">
        ...
        <div className="flex flex-wrap items-center gap-1 text-[10px] font-black">
          <span className="rounded-full bg-sky-100 px-2 py-1 text-sky-900 dark:bg-sky-500/20 dark:text-sky-100">
            일정 {counts.schedule}
          </span>
          <span className="rounded-full bg-amber-100 px-2 py-1 text-amber-950 dark:bg-amber-500/20 dark:text-amber-100">
            업무 {counts.task}
          </span>
          {counts.system > 0 && (
            <span className="rounded-full bg-rose-100 px-2 py-1 text-rose-950 dark:bg-rose-500/20 dark:text-rose-100">
              점검 {counts.system}
            </span>
          )}
        </div>
      ...
      <div
        className="flex flex-wrap gap-1 border-b border-slate-200 px-3 py-1.5 dark:border-white/10"
        role="tablist"
        aria-label="오늘 브리핑 분류"
      >
        {TABS.map((item) => (
          <button
            key={item.id}
            ...
            className={clsx(
              ...
              tab === item.id
                ? "border border-sky-300 bg-sky-100 text-sky-950 shadow-sm"
          ...
          {visible.map((item) => {
            ...
            const content = (
              <>
                <span
                  className={clsx(
                    "flex shrink-0 items-center gap-1 rounded-md px-1.5 py-0.5 text-[9px] font-black",
                    style.badge,
                  )}
                >
                  <Icon size={10} />
                ...
                <span className="min-w-0 flex-1">
                  ...
                  {item.detail && (
                    <span className="block truncate text-[9px] font-semibold text-slate-600 dark:text-slate-300">
                      {item.detail}
                    </span>
                  )}
                </span>
            ...
            return item.page ? (
              <button
                ...
                className="flex min-h-12 items-center gap-2 bg-white px-3 py-2 text-left hover:bg-slate-50 dark:bg-surface-900 dark:hover:bg-white/5"
              >
                {content}
              </button>
            ) : (
              <div
                key={item.id}
                className="flex min-h-12 items-center gap-2 bg-white px-3 py-2 dark:bg-surface-900"
              >
                {content}
              </div>
            );
