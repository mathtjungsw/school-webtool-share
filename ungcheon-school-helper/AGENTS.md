...
- 모바일에서는 관리자 PC가 공유 `NEIS급식` 시트에 동기화한 요청 기간 `meals`와 오늘의 하위 호환 `todayMeals`만 재사용한다. NEIS API 직접 호출·NEIS 학사일정·NEIS 학급시간표를 모바일 응답이나 캐시에 추가하지 않는다. 3학년 출결은 서버가 공유 학생시간표에서 로그인 교사의 해당 교시 실제 수강생만 대조한다. 일정 묶음에는 최소 출결 요약만 전달하고, 사용자가 출결 버튼을 누른 경우에만 별도 인증 API로 그 수업의 반·번호·이름·비고를 전달한다. 상세 명렬은 서비스 워커·장기 캐시에 저장하지 않으며 학번 원본·전체 학생시간표·원본 payload·다른 수업 자료는 전달하지 않는다.
- 날짜별 수업 운영 변경은 공유 `일일시간표예외`와 `timetableOverrides` 계약으로 데스크톱·모바일에 함께 반영한다. 모바일은 활성 예외를 읽기만 하며 관리자 변경 기능을 포함하지 않는다.
