...
- 모바일에서는 관리자 PC가 공유 `NEIS급식` 시트에 동기화한 요청 기간 `meals`와 오늘의 하위 호환 `todayMeals`만 재사용한다. NEIS API 직접 호출·NEIS 학사일정·NEIS 학급시간표를 모바일 응답이나 캐시에 추가하지 않는다. 3학년 출결은 서버가 공유 학생시간표에서 해당 교사의 실제 수강생만 대조하고, 모바일에는 비고가 있는 학생의 반·번호·이름·비고와 반별 입력 상태만 전달한다. 전체 학생 명렬·전체 학생 시간표·학번·원본 payload는 전달하지 않는다.
