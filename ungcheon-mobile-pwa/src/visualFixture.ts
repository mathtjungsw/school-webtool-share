...
export function visualFixture(today: string): DashboardPayload {
  ...
  return {
    ...
    changes: [],
    bundle: { teacherTimetable: null, committeeEvents: [], timetableChanges: [], fetchedAt: new Date().toISOString(), contractVersion: 3, sourceStatus: Object.fromEntries(['weekly', 'creative', 'gateDuty', 'mealDuty', 'timetable', 'committee', 'changes', 'overrides', 'meals', 'attendance'].map(key => [key, { state: 'fresh' as const, lastAttemptAt: new Date().toISOString(), lastSuccessAt: new Date().toISOString(), itemCount: 1 }])), meals, todayMeals: meals.filter(meal => meal.date === today), events: [{ date: today, title: '교직원 협의회 및 부서별 전달사항', source: 'weekly', label: '교무기획부' }, { date: today, title: '동아리 활동(5·6교시)', source: 'creative', label: '창체' }, { date: today, title: '등교지도 · 정문', source: 'gateDuty', label: '등교지도', time: '08:15~08:25' }] },
